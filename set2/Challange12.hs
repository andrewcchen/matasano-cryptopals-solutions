...
detectBlockSize blackbox = go 4
    where guessCorrect guess = uncurry (==) outputs
              where genInputs x = map (V.replicate $ guess + x) [0..255]
...
detectContentLength blackbox = upperBound - (go 0)
    where upperBound = V.length $ blackbox V.empty
...
decryptContent blackbox = go 0 []
    where contentLength = detectContentLength blackbox
          go offset acc = if offset < contentLength then go (offset + 1) newAcc
                                                    else acc
              where padCount = 15 - (offset `rem` 16)
