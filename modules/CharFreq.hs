...
byteToLower byte
    ...
    | otherwise = byte
    where isUpperCase = (charCode 'A') <= byte && byte <= (charCode 'Z')
...
scoreCharFreq :: ByteVector -> Double
scoreCharFreq bytes = totalPenalty / totalChars
    where charFreqs = countCharFreq $ V.map byteToLower bytes
          ...
          penalizeOccurWithThresh thresh cat = countCatChars cat - allowedOccurs
              where allowedOccurs = thresh * totalChars
          ...
          penalizeFreqAbsErr expProbs cat = V.sum $ V.map abs $ V.zipWith (-) expectedFreqs actualFreqs
              where catChars = fromIntegral $ countCatChars cat
                    expectedFreqs = V.map (catChars *) expProbs
